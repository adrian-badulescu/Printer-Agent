...
**Production hub (`10.60.0.0/24`, 3-VPS):** [PRODUCTION_HUB.md](PRODUCTION_HUB.md) — single model (`base` = Interface only, all peers in `peers.d/`), migration from legacy hubs.
...
- `/etc/wireguard/wg0.base.conf` (**Interface only** — no `[Peer]` blocks; see [PRODUCTION_HUB.md](PRODUCTION_HUB.md))
- `/etc/wireguard/wg0.conf` (used by `wg-quick up wg0` / `wg-quick@wg0` systemd service; copy of `base`)
- `/etc/wireguard/peers.d/` (all peers: `infra-*.conf` static + `<agentId>.conf` from backend)
- `/etc/wireguard/wg0.generated.conf` (generated by `wg-rebuild-generated`; applied by upsert/delete/sync)
- `/usr/local/bin/wg-rebuild-generated` (validate base, merge peers, `wg-quick strip` test)
- `/usr/local/bin/wg-sync-peers` (rebuild + apply `wg0.generated.conf` at boot)
- `/usr/local/bin/wg-migrate-legacy-hub` (one-time: extract `[Peer]` from legacy `base` → `peers.d/`)
- `install-wireguard-hub-scripts.sh` — copy all scripts to `/usr/local/bin/` (strips CRLF)
...
- `/usr/local/bin/wg-rebuild-generated` — validate `base`, merge `peers.d` → `wg0.generated.conf`
...
- `/usr/local/bin/wg-sync-peers` — rebuild + apply peers at boot (`PostUp`)
- `/usr/local/bin/wg-migrate-legacy-hub` — one-time legacy hub migration (production)

From the directory that contains the script files on the server (or after `scp` from Windows):
...
Do **not** run `sudo bash install-wireguard-hub-scripts.sh` directly after `scp` from Windows — CRLF breaks `set -o pipefail`. The install script strips CRLF from all `wg-*` files when piped through `sed`.

Alternative without the install script:

```bash
cd /tmp/wg-scripts
for name in wg-rebuild-generated wg-migrate-legacy-hub wg-sync-peers wg-peer-upsert wg-peer-delete; do
  sed 's/\r$//' "$name" | sudo tee "/usr/local/bin/${name}" >/dev/null
...
```bash
ls -l /usr/local/bin/wg-rebuild-generated /usr/local/bin/wg-peer-upsert \
  /usr/local/bin/wg-peer-delete /usr/local/bin/wg-sync-peers /usr/local/bin/wg-migrate-legacy-hub
head -1 /usr/local/bin/wg-rebuild-generated | cat -A   # expect: #!/usr/bin/env bash$  (no ^M)
```
...
`wg-peer-upsert` calls **`wg-rebuild-generated`**, which copies **`wg0.base.conf`** (Interface only) plus `peers.d/*.conf` into `wg0.generated.conf`. Never put `[Peer]` blocks in `base` — use `peers.d/infra-*.conf` for static VPS/dev peers ([PRODUCTION_HUB.md](PRODUCTION_HUB.md)). Keep `PostUp = /usr/local/bin/wg-sync-peers wg0` in `wg0.base.conf`.
...
| `AllowedIP ... [Peer]` on agent enroll | `[Peer]` blocks or typo in `wg0.base.conf` | Run `wg-migrate-legacy-hub`; see [PRODUCTION_HUB.md](PRODUCTION_HUB.md) |
| `wg-rebuild-generated` refuses `[Peer]` in base | Legacy all-in-one hub config | `wg-migrate-legacy-hub` or move peers to `peers.d/` manually |
