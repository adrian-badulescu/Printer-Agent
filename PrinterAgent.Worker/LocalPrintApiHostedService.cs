...
public sealed class LocalPrintApiHostedService : BackgroundService
{
    ...
    private async Task HandleRequestAsync(HttpListenerContext ctx, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var ok = await _printHandler.PrintAsync(job, cancellationToken).ConfigureAwait(false);
            if (ok)
            {
                await WriteJsonAsync(ctx.Response, HttpStatusCode.OK, new { success = true }).ConfigureAwait(false);
            }
            else
            {
                await WriteJsonAsync(ctx.Response, HttpStatusCode.InternalServerError, new { message = "Print failed." }).ConfigureAwait(false);
            }
        }
        catch (Exception ex)
        {
