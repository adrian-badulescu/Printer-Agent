# Signs any Authenticode-capable file (MSI, EXE, etc.).
# Modes:
#   SelfSigned  - creates or reuses CN=URS Printer Agent Self-Signed in CurrentUser\My (code signing).
#   Enterprise  - uses first cert in CurrentUser\My whose subject matches CN=U.R.S.
...
if (-not $result) {
    throw "Authenticode signing produced no result for $Path"
}
